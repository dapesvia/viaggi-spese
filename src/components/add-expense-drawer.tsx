...
export function AddExpenseDrawer({ open, onOpenChange }: AddExpenseDrawerProps) {
  ...
  const [payer, setPayer] = useState<"alex" | "tina">("alex"); // New State
  const [manualAlex, setManualAlex] = useState(""); // New State
  const [manualTina, setManualTina] = useState(""); // New State
  const [description, setDescription] = useState("");
  const [expenseDate, setExpenseDate] = useState(getInitialDate());

  // ... (useEffect for date sync unchanged)

  const [receiptImage, setReceiptImage] = useState<string | null>(null);
  const [loading, setLoading] = useState(false);

  // ... (handleImageSelect, convertToEur unchanged)
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (splitType === 'custom') {
        ...
        // Validation: sum should match amountInEur (approx)
        const sum = (expenseData.split_manual_alex + expenseData.split_manual_tina);
        if (Math.abs(sum - amountInEur) > 0.1) {
          alert(`Attenzione: La somma delle quote (€${sum.toFixed(2)}) non corrisponde al totale (€${amountInEur.toFixed(2)})`);
          setLoading(false);
          return;
        }
      }
      ...
      const payerName = payer === 'alex' ? 'Alex' : 'Tina';
      await logActivity(
        currentTrip.id,
        "create",
        "expense",
        `${payerName} ha aggiunto €${amountInEur.toFixed(2)} per ${catLabel}`,
        undefined,
        { amount: amountInEur, category, description, payer }
      );
  ...
  // Auto-fill manual inputs when amount changes, if needed
  useEffect(() => {
    if (splitType === 'custom' && amountInEur > 0 && !manualAlex && !manualTina) {
      setManualAlex((amountInEur / 2).toFixed(2));
      setManualTina((amountInEur / 2).toFixed(2));
    }
  }, [amountInEur, splitType]);
      ...
      <Drawer.Portal>
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto px-4 pb-6">
            ...
            {/* Footer */}
            <div className="p-4 border-t border-border/50 backdrop-blur-xl bg-background/50 mt-auto">
              <motion.button
                whileTap={{ scale: 0.98 }}
                onClick={handleSave}
                ...
                className={cn(
                  ...
                  !amount
                    ...
                    : "bg-primary text-primary-foreground hover:shadow-primary/25 hover:brightness-110"
                )}
              >
                {loading ? (
                  <>
